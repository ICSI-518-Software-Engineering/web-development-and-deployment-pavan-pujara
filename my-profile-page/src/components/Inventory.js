...
function App() {
  ...
  const fetchItems = async () => {
    try { 
      const response = await axios.get(process.env.REACT_APP_API_URI+'/items');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(process.env.REACT_APP_API_URI+`/items/${editingId}`, formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
          },
        });
      } else {
        await axios.post(process.env.REACT_APP_API_URI+'/items', formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
          },
        });
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(process.env.REACT_APP_API_URI+`/items/${id}`);
